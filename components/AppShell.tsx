...
export async function AppShell({actor,children}:{actor:Actor;children:React.ReactNode}){
  ...
  const home=actor.mustChangePassword?"/tai-khoan":(visible[0]?.href||"/khong-co-quyen");
  return <div className="app-shell"><aside className="sidebar">
    <Link className="brand sidebar-brand" href={home}><span className="brand-mark">LT</span><span>{site.brandName}</span></Link>
    <small className="account-label">{actor.name} · {roleLabels[actor.role]}</small>
    {actor.mustChangePassword?<p className="account-label">Đổi mật khẩu tạm trước khi dùng hệ thống.</p>:<nav>{visible.map(item=><Link key={item.key} href={item.href}>{item.label}</Link>)}</nav>}
    {!actor.mustChangePassword&&alternate&&<Link className="portal-switch" href={alternate.href}>{preferredScope==="cms"?"Mở cổng cá nhân":"Mở khu quản trị"} ↗</Link>}
